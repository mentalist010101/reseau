...
export class GlCommitDetails extends GlDetailsBase {
	...
	@state()
	get wipState() {
		if (this.state?.wip == null) return undefined;

		const { changes, branchName } = this.state.wip;
		return {
			changes: changes,
...
[⌥] Pick Commit..."
					...
					${when(
						this.details != null && !this.isStash,
						() => html`
							<ul class="top-details__authors" aria-label="Authors">
								<li class="top-details__author" data-region="author">
									<commit-identity
										name="${this.details!.author.name}"
										email="${this.details!.author.email}"
										date=${this.details!.author.date}
										dateFormat="${this.preferences?.dateFormat}"
										avatarUrl="${this.details!.author.avatar ?? ''}"
										showAvatar="${this.preferences?.avatars ?? true}"
										actionLabel="${this.details!.sha === uncommittedSha ? 'modified' : 'committed'}"
									></commit-identity>
								</li>
							</ul>
						`,
